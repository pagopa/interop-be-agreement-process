  ...
  def suspend(
    ...
    for {
      ...
      updateSeed                = AgreementManagement.UpdateAgreementSeed(
        ...
        stamps = agreement.stamps
          .copy(
            suspensionByConsumer = if (!suspendedByConsumer.getOrElse(false)) None else suspensionByConsumerStamp,
            suspensionByProducer = if (!suspendedByProducer.getOrElse(false)) None else suspensionByProducerStamp),
