...
final case class AgreementManagementServiceImpl(invoker: AgreementManagementInvoker, api: AgreementApi)(implicit
  ...
  )(agreementId: String, verifiedAttributeSeed: VerifiedAttributeSeed): Future[Agreement] = for {
  ...
  override def activateById(
    contexts: Seq[(String, String)]
  )(agreementId: String, stateChangeDetails: StateChangeDetails): Future[Agreement] = for {
    (bearerToken, correlationId, ip) <- extractHeaders(contexts).toFuture
  ...
  override def upgradeById(
    contexts: Seq[(String, String)]
  )(agreementId: UUID, agreementSeed: AgreementSeed): Future[Agreement] = for {
    (bearerToken, correlationId, ip) <- extractHeaders(contexts).toFuture
  ...
  override def getAgreementById(contexts: Seq[(String, String)])(agreementId: String): Future[Agreement] = for {
    (bearerToken, correlationId, ip) <- extractHeaders(contexts).toFuture
